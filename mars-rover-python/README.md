...
Implement the solution so all tests pass. The tests are in `test_rover_ride.py`.

You will need to create these modules (with the classes/names the tests expect):

- **rover** – class `Rover`
- **plateau** – class `Plateau`
- **position** – class `Position`
- **instructions** – class `Instructions`, and `Instruction` (enum or constants: LEFT, RIGHT, MOVE)
- **exceptions** – `PositionNotOnPlateauException`, `NotDroppedException`, `UnknownInstructionException`
